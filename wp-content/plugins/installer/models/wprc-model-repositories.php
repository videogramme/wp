 ...
 */     
    public function createRepositoryTable()
    {       
        $q = " CREATE TABLE IF NOT EXISTS $this->repository_table (
              ...
              `repository_description` TEXT DEFAULT '',
    ...
    public function upgrade_db() {
        $q = "SHOW COLUMNS FROM $this->repository_table";
        ...
        if ( ! in_array( 'repository_logo', $columns ) )
            $add_query[] = "ADD `repository_logo` varchar(255) DEFAULT ''";
        if ( ! in_array( 'repository_description', $columns ) )
            $add_query[] = "ADD `repository_description` TEXT DEFAULT ''";
        if ( ! in_array( 'repository_website_url', $columns ) )
            $add_query[] = "ADD `repository_website_url` varchar(255) DEFAULT ''";
        if ( ! in_array( 'requires_login', $columns ) )
            $add_query[] = "ADD `requires_login` tinyint(1) DEFAULT 0";

        $result = false;
        if ( count( $add_query ) > 0 ) {
            $add_query = implode( ',', $add_query);
            $q = "ALTER TABLE $this->repository_table " . $add_query;
        ...
        if ( $result ) {
            ...
            foreach ( $predef_repos as $repository ) 
                $this -> updateRepositoryUpgraded(
                    $repository['repository_logo'],
                    $repository['repository_website_url'],
                    $repository['repository_description'],
                    $repository['requires_login'] 
                );
        }
        
    }

    public function updateRepositoryUpgraded( $repository_id, $repository_logo, $repository_site_url, $repository_description, $repository_requires_login )
    {     
